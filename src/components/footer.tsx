...
const Footer: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentWord(prev => (prev + 1) % agents.length)
    }, 2000)
  ...
  return (
    <footer className="p-8 text-center font-mono">
      <div
        id="social"
        className="text-center [&_a]:m-1 [&_a]:text-[xx-large] [&_a]:inline-block [&_a]:relative [&_a_svg]:transition-all [&_a_svg]:duration-300 [&_a_svg]:ease-[cubic-bezier(0.4,0,0.2,1)] [&_a_svg]:drop-shadow-[0_0_0px_transparent] hover:[&_a_svg]:[fill:var(--link-hover-color)] hover:[&_a_svg]:drop-shadow-[0_0_8px_var(--link-hover-color)] hover:[&_a_svg]:rotate-[-5deg] hover:[&_a_svg]:scale-105"
      >
        ...
        <Icon
          ...
          label="StackOverflow"
        />
        <Icon url="https://dev.to/aashutoshrathi" label="Dev" />
